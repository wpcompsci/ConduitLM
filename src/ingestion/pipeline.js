  ...
  scope.Pipeline = {
    handleListNotebooks: async function () {
      try {
        ...
        throw e.code
          ? e
          : { code: 'notebook_list', message: e.message || 'Failed to list notebooks' };
      }
    },
    ...
    handleIngest: async function (destination) {
      ...
      if (destination.type === 'create') {
        try {
          ...
          throw e.code
            ? e
            : { code: 'notebook_create', message: e.message || 'Failed to create notebook' };
        }
      }
      ...
      try {
        ...
        throw e.code
          ? e
          : { code: 'source_ingest', message: e.message || 'Failed to add text source' };
      }
    },

    handleSaveConversation: async function (data) {
      const { conversation, source } = data; // notebookId currently unused
      ...
      try {
        ...
        if (isGDocImport) {
          throw {
            ...
            message: 'GDoc import not fully implemented in pipeline yet.',
          };
        } else {
          // scope.sourceFormatters must be loaded
          if (!scope.sourceFormatters)
            throw { code: 'dependency', message: 'Source formatters not loaded' };
        ...
        throw e.code
          ? e
          : { code: 'save_conversation', message: e.message || 'Failed to save conversation' };
      }
    },
  };
})(globalThis);
